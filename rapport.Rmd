...
Chaque joueur a ses forces et ses faiblesses, au fur et à mesure de son évolution. Nous nous demanderons donc ce qui démarque un joueur pro d'un joueur amateur, sur quels domaines un joueur professionnel est il plus performant ? Est ce que chaque joueur a sa spécialité poussée au maximum ? Est-ce que certains aspects changent peu entre un joueur professionnel et un joueur amateur ? Pour le savoir nous allons comparer les statistiques de jeu des deux catégories de joueurs.

```{r include=FALSE}
# Amateur
amateurs <- read_csv("data/4-Stats and Ranks of 2600 Valorant Players.csv")
head(amateurs)
...
pros <- bind_rows(
  perfs_2021 %>% mutate(annee = 2021),
  perfs_2022 %>% mutate(annee = 2022),
  perfs_2023 %>% mutate(annee = 2023),
  perfs_2024 %>% mutate(annee = 2024),
  perfs_2025 %>% mutate(annee = 2025)
)
head(pros)

stats <- bind_rows(
  pros %>% mutate(categorie = "pros"),
  amateurs %>% mutate(categorie = "amateurs")
)
head(stats)

```

Tout d'abord, regardons les kills par rapport aux morts des joueurs. Un joueur professionnel meurt il moins souvent qu'un joueur amateur ?

```{r}
ggplot(data = pros, aes(x = Kills, y = Deaths)) +
  geom_point(color = rgb(0.17, 0.21, 0.35)) +
  labs(
    x = "Nombre de Kills",
    y = "Nombre de Morts",
    title = "Nombre de Morts par nombre de Kills chez les professionnels"
  )

ggplot(data = amateurs, aes(x = kills, y=deaths)) +
  geom_point(color = rgb(0.17, 0.21, 0.35)) +
  labs(
    x = "Nombre de Kills",
    y = "Nombre de Morts",
    title = "Nombre de Morts par nombre de Kills chez les amateurs"
  )

```

On peut observer que les joueurs amateurs ont un ratio Kills/Deaths beaucoup plus disparate que les joueurs professionnels, bien que les deux tendent à tourner autour d'un ratio de 1kills pour 1 mort. Nous allons donc observer la répartition du ratio Kills/Death.

```{r}
ggplot(data = amateurs, aes(x = kills/deaths)) +
  geom_density(
    aes(y = after_stat(count)), # Affiche le nombre de personnes
    fill = rgb(0.17, 0.21, 0.35), 
    color = rgb(0.17, 0.21, 0.35),
    linewidth = 1
  ) +
  labs(
    x = "Ratios Kills/Deaths",
    y = "Nombre de Joueurs",
    title = "Répartition du ratio Kills/Death chez les amateurs"
  )
ggplot(data = pros, aes(x = Kills/Deaths)) +
  geom_density(
    aes(y = after_stat(count)), # Affiche le nombre de personnes
    fill = rgb(0.17, 0.21, 0.35), 
    color = rgb(0.17, 0.21, 0.35),
  ...
  labs(
    x = "Ratios Kills/Deaths",
    y = "Nombre de Joueurs",
    title = "Répartition du ratio Kills/Death chez les professionnels"
    )+
  scale_x_continuous(
    limits = c(0, 5),
...
On peut observer que la majorité des joueurs ont effectivement un ratio Kills/deaths qui tourne autour de 1.On peut également observer qu'il y a davantage de joueurs qui ont un ratio au dela de 1 chez les professionnels.

Nous allons désormais nous concentrer sur une autre statistique importante du jeu : le pourcentage de tirs dans la tete. Cette statistique nous permettra d'observer la précision des joueurs professionnels et amateurs. Nous nous attendons à ce que les joueurs professionnels soient plus précis que les joueurs amateurs.

```{r}
ggplot(data = amateurs, aes(x = headshots/100)) +
  geom_density(
    aes(y = after_stat(count)), # Affiche le nombre de personnes
    fill = rgb(0.17, 0.21, 0.35), 
    color = rgb(0.17, 0.21, 0.35),
  ...
  labs(
    x = "Pourcentage de headshots",
    y = "Quantité de personnes",
    title = "Nombre de joueurs par pourcentage de headshots chez les amateurs"
  )
ggplot(data = pros %>% mutate(`Headshot %` = parse_number(`Headshot %`)), aes(x = `Headshot %`)) +
  geom_density(
    aes(y = after_stat(count)), # Affiche le nombre de personnes
    fill = rgb(0.17, 0.21, 0.35), 
    color = rgb(0.17, 0.21, 0.35),
  ...
  labs(
    x = "Pourcentage de Headshots",
    y = "Quantité de personnes",
    title = "Nombre de joueurs par pourcentage de Headshots chez les professionnels"
  )

```

On peut observer que le pourcentage de Headshot semble plus réparti au niveau de 50% pour les joueurs amateurs tandis que les joueurs professionnels tournent davantage autour de 25% de headshots. Cela s'explique notamment car les résultats des joueurs amateurs ont des résultats beaucoup plus disparates du à la différence de niveau et ils sont peu nombreux, avec un maximum d'un peu moins de 125 personnes au niveau du pic alors que les joueurs professionnels sont beaucoup plus nombreux, ils sont 20000 au niveau du pic et les résultats sont globalement plus lisses car les joueurs professionnels ont tous un niveau similaire. Le pourcentage de headshots est également plus bas pour les joueurs professionnels aussi potentiellement car les joueurs professionnels ont des réflexes et des capacités d'esquives plus développées ce qui diminue la probabilité de toucher la tête.

## Interprétation

Pour conclure, les joueurs professionnels ont des résultats plus lisses et un ratio Kills/Deaths légèrement plus important que les amateurs. Leur pourcentage de Headshots est plus faible mais cela peut s'expliquer par les réflexes accrus des joueurs professionnels, facilitant l'esquive et augmentant la difficulté du Headshot.
...
### Graphiques des meilleurs joueurs de chaque année
...
ggplot(top_players_2021, aes(x = reorder(Player, -Rating), y = Rating)) + geom_bar(stat = "identity", fill = "#ff4654") + labs(title = "Bar chart des meilleurs joueurs de 2021", x = "Joueur", y = "Rating") #Graphique
...
ggplot(top_players_2022, aes(x = reorder(Player, -Rating), y = Rating)) + geom_bar(stat = "identity", fill = "#ff4654") + labs(title = "Bar chart des meilleurs joueurs de 2022", x = "Joueur", y = "Rating") #Graphique
...
ggplot(top_players_2023, aes(x = reorder(Player, -Rating), y = Rating)) + geom_bar(stat = "identity", fill = "#ff4654") + labs(title = "Bar chart des meilleurs joueurs de 2023", x = "Joueur", y = "Rating")#Graphique
```
...
ggplot(top_players_2024, aes(x = reorder(Player, -Rating), y = Rating)) + geom_bar(stat = "identity", fill = "#ff4654") + labs(title = "Bar chart des meilleurs joueurs de 2024", x = "Joueur", y = "Rating") #Graphique
...
ggplot(top_players_2025, aes(x = reorder(Player, -Rating), y = Rating)) + geom_bar(stat = "identity", fill = "#ff4654") +
...
# 7 & 8. Quelles sont les meilleures équipes par année ?

Nous répondons aux questions 7 & 8 en simultané dans une question un peu plus grande car nous les considérons très complémentaires et plus faciels à étudier de cette manière.
...
ggplot(pct_data, aes(x = Total_Kills, y = Pct, color = Outcome, group = Outcome)) +
  ...
  scale_color_manual(values = c("Win" = rgb(0.17, 0.21, 0.35), "Loss" = "#ff4654")) +
...
ggplot(mock_matrix, aes(x = Next_Outcome, y = Outcome, fill = Prob)) +
  ...
  scale_fill_gradient2(low = "#FFFFFF", mid = "#FF949D", high = "#ff4654", midpoint = 0.45) +
...
ggplot(top_25_duels, aes(x = Player, y = Enemy, size = `Player Kills`, color = factor(Year))) +
  geom_point() + 
  scale_size(range = c(2, 12)) +
  scale_color_brewer(palette = "Set1") +
  labs(
    title = "Matrice des 25 pires Némésis (VCT 2021-2026)",
    x = "Tueur (Player)",
    y = "Victime (Enemy)",
    color = "Année"
  ) +
...
Enfin, nous avons choisi tout compte fait d'écarter certaines de nos questions qui étaient quelques fois trop amibitieuses comme la question "Qu'est-ce qui compte le plus pour gagner une partie ?": tout le monde aimerait pouvoir répondre simplement à cette question mais cela risque d'être impossible. De plus, certaines de nos questions nous ont finalement semblé peu pertinentes ou alors nos données n'étaient pas adaptées pour y répondre, comme les questions 14, 15 ou 20. Nous aurions aimé avoir eu plus de temps pour répondre à certaines, et nous aurions aimé trouver des questions plus pertinentes à la base même si cela était compliqué avec une connaissance limitée des données.

### Mots de la fin : 

Elio : J'ai beaucoup aimé découvrir la visualisation et la manipulation de données avec R et Tidyverse, qui ont été des outils très efficaces et largement satisfaisants pour mettre en pratique les parties théoriques du cours. L'analyse exploratoire s'est avérée être une démarche très intéressante et engageante. J'aurais aimé pouvoir davantage faire le lien en détail avec les compétences des agents et leur style de jeu, afin de  mieux comprendre certaines dynamiques.

Morgane : J'ai beaucoup aimé travailler sur ce sujet même si ma méconnaissance du jeu m'a un peu ralentie. Les autres membres du groupe m'ont beaucoup soutenue et m'ont aidé à comprendre le vocabulaire propre au jeu. Je suis fière d'avoir pu réaliser ce projet qui a été très instructif sur bien des niveaux.
...
Ilias : J'ai trouvé l'utilisation de R très pertinente pour analyser et travailler sur un cas technique comme celui des jeux de données Valorant sélectionnés. J'aurais aimé pouvoir analyser plus en détail les jeux de données, et pouvoir arriver à des conclusions expertes, et plus "intéressantes", mais cela nécessite des connaissances en analyse statistique et une connaissance du jeu qui vont au-delà des attentes dece cours. Il pourrait être intéressant de les creuser dans le cadre d'un projet personnel.

# Annexe: répartition des tâches

Elio : Questions 2, 5, 10, 17 et introduction + une partie de la structuration du présent rapport. Description d'une partie des datasets.

Morgane : Soutenance + Questions 1 et 3 + PowerBI + Description des datasets 6 et 7

Louis : Questions 4 et 12 + application shiny + conclusion + relecture interprétations + soutenance

Ilias : Questions 7&8, 9, 11.
