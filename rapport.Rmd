...
Commençons par comparer simplement le nombre de kills pour chaque agent.

```{r}
ggplot(blitzagents, aes(x = agent, y = kills)) +
  geom_col() +
  labs(
    x = "Agent",
    y = "Nombre de kills",
    title = "Nombre de kills par agent"
  ) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, vjust = .5, face = "bold"))
```

On remarque que les écarts semblent énormes: plusieurs dizaines de millions de différence entre Jett qui en a le plus, et Cypher qui en a le moins.
Cependant, il est très probable que certains agents soient beaucoup plus joués que d'autres (il y a une trentaine d'agents disponibles pour 5 joueurs par équipe). Cela serait à prendre en compte, car dans ce cas tout le reste des données serait biaisé. Faisons le même graphique mais en comptant cette fois le nombre de matchs joués pour chaque agent.

```{r}
ggplot(blitzagents, aes(x = agent, y = matchesPlayed)) +
  geom_col() +
  labs(
    x = "Agent",
    y = "Nombre de matchs",
    title = "Nombre de matchs par agent"
  ) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, vjust = .5, face = "bold"))
```

Les deux graphiques semblent pratiquement identiques! Cela montre bien que les agents sont choisis de manière très inégale, et que l'on ne pourra pas tirer de conclusions intéressantes si on ne prend pas en compte le nombre de matchs pour chaque agent. Il faut se débrouiller pour observer des moyennes ou des ratios de plusieurs façons. Commençons par une statistique importante et connue: le Kills/Deaths ratio, qui nous permettra de voir quels agents meurent plus qu'ils ne tuent en moyenne.
...
On retrouve encore une fois Jett dans le haut du classement. On a aussi Phoenix, Reyna et Chamber qui avaient tous de bons KD, tandis que Clove est plutôt une surprise puisqu'iel avait un KD dans la moitié la plus basse. Pourtant, Chamber ne se trouve même pas dans le top 5 alors qu'il avait le meilleur KD assez loin devant les autres. Il est probable que ce personnage meure beaucoup moins souvent que les autres, ou alors qu'il fasse assez souvent beaucoup de kills par round.
